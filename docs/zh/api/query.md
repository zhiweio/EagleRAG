...
!!! info "理论：RAG 的流式 UX"
    对话式检索研究（如检索增强系统中的*增量生成*）表明，用户在首个 token 前看到*进度*时，**感知延迟**显著下降。因此 Eagle-RAG 将 **step 事件**（route、recall、rerank）与 **token 事件**（LLM 增量）分离。Step 回答「系统在做什么？」；token 回答「答案正在变成什么？」。这与 HCI 文献中对长时 AI 任务的*渐进披露*模式一致（Shneiderman，1998）。

## 端点摘要

| 方法 | 路径 | 响应 | 用途 |
|--------|------|----------|---------|
| `POST` | `/query` | `QueryResponse` | 完整问答（route → retrieve → generate → persist） |
...
| `GET` | `/tags` | `TagListResponse` | 范围过滤 UI 的关键词标签目录 |

会话路由（`/sessions/*`）见 [会话](sessions.md)。

!!! note "多集合检索"
    Core 查询命中实例绑定的 Milvus Database 中的 `eagle_text` 和/或 `eagle_visual`。域 profile 可通过 `QueryRouteClassifier` + `RetrieverOrchestrator`（RRF 合并）增加专用集合。Core 默认永不自动查询专用集合（G4）。参见 [插件架构](../architecture/plugin-architecture.md)。
...
| 字段 | 类型 | 必填 | 描述 |
|-------|------|----------|-------------|
| `session_id` | `string \| null` | 否 | 已有会话 UUID。省略 → 自动创建，标题为查询前 30 字符 |
| `query` | `string` | **是** | 自然语言问题 |
| `mode` | `auto \| text \| visual \| hybrid` | 否 | 覆盖路由。默认来自 `settings.router.mode` |
| `kb_name` | `string \| null` | 否 | 单 KB 遗留范围。`scope_filter` 非空时忽略 |
| `attachments` | `string[] \| null` | 否 | 来自 `POST /attachments` 的 `attachment_id` |
| `scope` | `string[] \| null` | 否 | 遗留 `document_id` 列表（`scope_filter` 未激活时检索后过滤） |
| `filters` | `QueryFilters \| null` | 否 | Milvus 标量面：`source_type`、`pipeline`、`year` |
| `scope_filter` | `ScopeSelection \| null` | 否 | 高级并集范围（见下） |
...
**并集（OR）语义：** 块在以下**任一**成立时合格：

1. 其 `kb_name` 在 `kb_names` 中
2. 其 `document_id` 在 `document_ids` 中
3. 其文档在**任一**所选标签解析出的文档集中

空 `ScopeSelection` → 回退遗留 `kb_name` / `scope` 行为。

### `SearchRequest`

与 `QueryRequest` 相同字段，**但**无 `session_id` 与 `attachments`。用于基准与前端「搜索模式」（证据轨无生成）。
...
| `steps` | `QueryStep[]` | 执行轨迹（`route`、`recall`、`rerank`、`warning`、…） |
...
| `path` | Knowhere 层次 `path` |
| `content` | 块正文（表格含 HTML） |
| `summary`, `keywords`, `page_nums` | 语义元数据 |
| `document_id`, `file_name`, `file_path` | 文档锚点 |
| `kb_name`, `source_type` | 多租户 + 面 |
| `source` | `kb \| attachment` |
| `score` | 重排后相关性 |

### `ImageSource`

带**四个融合锚定字段**的视觉瓦片（见 [多模态融合](../architecture/multimodal-fusion.md)）：

| 字段 | 用途 |
|-------|---------|
| `chunk_type` | `tile \| image \| table` |
| `parent_section` | 最近文本块 `path` |
...
## Scope filter → Milvus 下推 {#scope-filter--milvus-pushdown}
...
标签解析跨**所有知识库**（标签为 `document_keywords` 中的全局关键词）。上限 `settings.router.max_scope_documents` 防止无界 `document_id in […]` 表达式。

!!! warning "遗留 `scope` vs `scope_filter`"
    `scope_filter` 激活时，遗留 `scope` 列表**不**在检索后应用。请将文档约束放在 `scope_filter.document_ids` 中。
...
每个事件遵循 [HTML Living Standard SSE 文法](https://html.spec.whatwg.org/multipage/server-sent-events.html)：
...
- `data` 为 `json.dumps(..., ensure_ascii=False)` 序列化的**单个 JSON 对象**。
- **不**使用多行 `data:`；载荷单行。
- 今日不发射 `id:` 或 `retry:`。
...
| 事件 | 时机 | `data` 形状 |
|-------|------|--------------|
| `session` | `session_id` 已知后首次 yield | `{ session_id, user_message_id }` |
| `step` | Route / recall / rerank / attach-parse | `{ name, … }` — 允许额外键 |
| `sources` | 重排后、生成前 | `QuerySources` 对象 |
| `token` | 每个 LLM 增量 | `{ delta: string }` |
| `done` | 持久化后 | 完整载荷 + `message_id` |
...
| `database_unavailable` | 503 | 会话存储宕机 |
...
3. 将 `token.delta` 追加到运行中答案缓冲。
4. 在 `sources` 时替换待定 UI 状态（答案完成前即可见引用）。
5. `done` 时用 `message_id` 替换临时消息 id。
6. 连接断开：无自动恢复；重新 `POST` 为新查询。

!!! tip "引用 UI 模式"
    在 `token` 事件**之前**发射 `sources` 实现 Perplexity 式*证据优先*：答案流式输出时用户可检查召回块。`SourcesPanel` 如何绑定此事件顺序见 [问答模块](../frontend/qa-module.md)。
...
| `scope_filter.kb_names` 非空 | 所列 KB 并集；顶层 `kb_name` 检索时忽略 |
| 会话创建 | `kb_name` 存于会话行 |
| 消息持久化 | `kb_name` 从请求复制 |
...
- [MCP 工具](mcp-tools.md) — `core_query` 工具模式
