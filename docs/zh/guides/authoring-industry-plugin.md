...
1. `plugins/<namespace>/` — 实现 `Plugin` 协议（可从 `plugins/_template/` 复制）
...
Biomed 评测套件：`eval/biomed/` — `RETRIEVAL.md`、`EVAL.md`。
...
- `plugins/biomed`（**实验性**）- 专用 collection + 编码器 + 实体锚定检索 hook + IMRaD CHUNK enrich。完整深入：[Biomed 插件](../architecture/biomed-plugin.md) + [Biomed 检索](../architecture/biomed-retrieval.md)；评测见 `eval/biomed/`
