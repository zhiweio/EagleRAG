...
`api` 与 `worker-pixelrag` 挂载 `./data:/app/data` 并设置 HuggingFace 镜像环境变量，以便容器内下载 Qwen3-VL 模型。
...
| `docker/Dockerfile.worker` | Python 3.12 slim + PixelRAG 用 Chrome | Celery worker 入口读 `QUEUES` / `CONCURRENCY` |
