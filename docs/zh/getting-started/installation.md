...
| `uv sync --extra docs` | MkDocs Material | 本地文档站 |

```bash
uv sync                  # 核心（必需）
uv sync --group dev      # 可选：测试/lint
uv sync --extra docs     # 可选：文档
```
...
| 视觉嵌入（2048 维） | Qwen3-VL-Embedding-2B | 经 PixelRAG 本地 —— 无 API 密钥 | `_Qwen3VLVisualEncoder` |
...
- `pixelrag_adapter.py` 惰性导入 —— 缺失则快速失败
- `embedding.visual.provider` 必须为 `"pixelrag"`，否则 `_ensure_loaded()` 抛错
...
| 主题 | 说明 |
| --- | --- |
| 视觉编码器惰性加载 | `_Qwen3VLVisualEncoder` 在首次 `embed_tiles` 时加载 — API 容器可无 GPU 启动；首个 pixelrag 任务承担模型加载 |
| Knowhere 轮询窗口 | SDK 在 worker 内最长阻塞 `knowhere.poll_timeout`（默认 1800s）— 非 API 超时 |
| 嵌入提供方锁定 | `embedding.visual.provider` 须为 `pixelrag` — 第三方视觉 API 会改变向量几何并破坏 Milvus 索引 |
...
| DashScope | `DASHSCOPE_API_KEY`、`TEXT_EMBEDDING_MODEL`、`RERANK_TEXT_MODEL` | 嵌入 + 重排 |
