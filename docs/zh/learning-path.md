...
Eagle-RAG 在单集群内使用**两层隔离**：

| 层 | 标识符 | 机制 |
| --- | --- | --- |
| 域 | `plugin_namespace` | Milvus Database + PostgreSQL 仓库过滤（部署时） |
...
去重键 `(sha256, kb_name, plugin_namespace)` 允许同一文件存在于多 KB 与域（不同实例上）。详见 [多租户](architecture/multi-tenancy.md)。
...
- [ ] [插件架构](architecture/plugin-architecture.md)
- [ ] [ADR-008 纯 RAG + 前端范围](architecture/adr/008-rag-only-plugin-platform.md)
...
| 跨租户泄漏 | 缺少 `kb_name` 过滤或错误的 `plugin_namespace` / profile | [多租户](architecture/multi-tenancy.md) |
| 重复上传被拒 | 去重命中 `(sha256, kb_name, plugin_namespace)` | [入库 API](api/ingest.md) |
