...
> 面向 Agent 与 LLM 的、行业无关、多租户（`kb_name`）**多模态检索增强生成（RAG）** 知识库 — **数据层**（非业务 Agent 应用）。

!!! important "产品边界"
    Eagle-RAG 负责入库、检索、上下文组装与溯源，经 REST/SSE/MCP 交给下游 Agent。内置前端只展示 **Core** knowhere + pixelrag；垂类插件（biomed、lakehouse-bi 等）**仅后端 MCP**。见 [ADR-008](architecture/adr/008-rag-only-plugin-platform.md)、[插件架构](architecture/plugin-architecture.md)。
...
- :octicons-organization-24: **多租户** — 两层：`plugin_namespace`（域 / Milvus Database）与 `kb_name`（该域内 KB）；去重 `(sha256, kb_name, plugin_namespace)`。
...
- :octicons-plug-24: **MCP 工具服务** — `/mcp` 提供 `core_ingest`、`core_query`、`core_retrieve_text`、`core_retrieve_visual`（[Model Context Protocol](https://modelcontextprotocol.io/)）；领域插件按 profile 追加 `{namespace}_*`。
- :octicons-puzzle-24: **微内核插件** — Core + 同仓垂类插件；`EAGLE_RAG_PROFILE` 切换部署域；二开见 [编写行业插件](guides/authoring-industry-plugin.md)。
...
| 关注点 | 关键设置 | 环境变量 |
| --- | --- | --- |
| 默认租户 / 域 | `kb_name`、`plugins.default_namespace` | `KB_NAME`、`EAGLE_RAG_PROFILE`、`PLUGIN_NAMESPACE` |
| Milvus | `milvus.host`、`milvus.db_name`、`visual_index_type` | `MILVUS_HOST`、`MILVUS_VISUAL_INDEX_TYPE` |
| 插件 | `plugins.enabled`、`plugins.options` | `EAGLE_RAG_PROFILE` |
