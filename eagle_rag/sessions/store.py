...
_SESSION_COLUMNS = "session_id, title, created_at, updated_at, kb_name, scope_filter"
...
async def create_session(
    ...
    *,
    ...
    row = await async_fetchrow(
        "INSERT INTO sessions (session_id, title, kb_name, scope_filter) "
...
async def get_session(session_id: str) -> dict[str, Any] | None:
    ...
    row = await async_fetchrow(
        f"SELECT {_SESSION_COLUMNS} FROM sessions WHERE session_id = $1",
        session_id,
    )
...
async def list_sessions(
    *, kb_name: str | None = None, limit: int = 50, offset: int = 0
) -> list[dict[str, Any]]:
    """List sessions paginated, newest ``updated_at`` first; filter by ``kb_name`` when given."""
    if kb_name is None:
        rows = await async_fetch(
            f"SELECT {_SESSION_COLUMNS} FROM sessions ORDER BY updated_at DESC LIMIT $1 OFFSET $2",
            limit,
            offset,
        )
    else:
        rows = await async_fetch(
            ...
            "FROM sessions WHERE kb_name = $1 ORDER BY updated_at DESC LIMIT $2 OFFSET $3",
            kb_name,
            limit,
            offset,
        )
...
async def update_session(session_id: str, *, title: str | None = None) -> dict[str, Any]:
...
async def set_session_scope_filter(session_id: str, scope_filter: dict | None) -> None:
    """Persist the last-used scope filter for a session (for cross-device restore)."""
    await async_execute(
        "UPDATE sessions SET scope_filter = $1::jsonb WHERE session_id = $2",
        _dumps(scope_filter),
        session_id,
    )
...
async def delete_session(session_id: str) -> bool:
    ...
    result = await async_execute("DELETE FROM sessions WHERE session_id = $1", session_id)
...
async def add_message(
    ...
    *,
    ...
    async with acquire_async() as conn:
        ...
        await conn.execute(
            "UPDATE sessions SET updated_at = NOW() WHERE session_id = $1", session_id
        )
...
async def list_messages(
    ...
    rows = await async_fetch(
        f"SELECT {_MESSAGE_COLUMNS} FROM messages WHERE session_id = $1 "
...
async def get_message(message_id: str) -> dict[str, Any] | None:
    ...
    row = await async_fetchrow(
        f"SELECT {_MESSAGE_COLUMNS} FROM messages WHERE message_id = $1", message_id
    )
